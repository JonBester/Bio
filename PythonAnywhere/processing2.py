...
'''
def crispr_decon(gRNA, fasta, ab1, fasta_rv, guide_rv, bracket2, freshhold2, insert_length2, upstream_length2, downstream_length2, length2):
    output_strings = []
    ################################### INPUT PARAMETERS ######################################
...
####################################### CODE ############################################
    ...
    for i in range(0,len(wtstring)):
        if wtstring[i:i+len(GuideSeq)] == str(GuideSeq):
            CutSite = i+len(GuideSeq)-3
            print(CutSite)
            print(wtstring[i:i+len(GuideSeq)])
            break
    if CutSite == 0:
    ...
    #print("Trip1: trainer creation successful")
    #print(trainer)
    ...
    #print(f"FirstPeakLoc:{FirstPeakLoc} and FinalPeakLoc:{FinalPeakLoc}")
    if initial == 0:
    ...
    print("Trip3: deviation site identified")
    ...
    print("Trip5: wt string truncated created")
    print(first)
    print(second)
